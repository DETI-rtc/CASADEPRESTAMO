...
import LoaderGraph from "../components/paragraficos/l-graph.vue";
  ...
  methods: {
    ...
    searchRemoteItems(input) {
        ...
        if (input.length > 1) {
            ...
            axios.get(`/reports/data-table/items/?${parameters}`)
                    .then(response => {
    ...
    loadData() {
      ...
      axios.post(`/api/${this.resource}/data`, this.form).then(response => {
    ...
    //   axios.get(`/command/df`).then(response => {
