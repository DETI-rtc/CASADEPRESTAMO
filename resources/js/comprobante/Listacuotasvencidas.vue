    ...
    <div>
        <div class="card card-outline card-primary">
            <div class="card-header">
                <h3 class="card-title">Cuotas Venvcidas por mes</h3>
            ...
            <div class="card-body p-3">
                <v-client-table :data="cuotas_vencidas" :columns="columna1" :options="options1">
                    <div slot="fecha_ven" slot-scope="{row}">
                        {{row.fecha_ven|myDate}}
                    </div>
                                    <div slot="acciones" slot-scope="{ row }">
                                        <div class="btn-group">
                                            ...
                                            <!-- <template v-if="row.comprobante == null"> -->
                                                ...
                                                <button type="button" class="btn btn-success btn-sm" @click="PagoCompro(row)" v-if="row.tiene_comprobante == null " v-loading.fullscreen.lock="loading"><i class="fas fa-ticket-alt"></i></button>
                                            <!-- </template> -->
    ...
    export default {
        data() {
            return {
                ...
                mes:'',
                anio_cuotasv :momento().format('yyyy'),
                mes_cuotasv :parseInt(momento().format('M')),
                cuotas_vencidas:[],

                columna1: ['numero_credito','fecha_ven','nom_ape','cuotanro','sal_cap','cap_amor','interes','seg_des','com_des', 'cuota','tiene_comprobante','acciones'],
                options1: {
                    headings: {
                    },
                    perPage: 15,
                ...
                Swal.fire({
                    ...
                    if (result.value == true) {
                        this.loading = true;
                            ...
                            .then(({data}) => {
                                this.rutapdf='http://admin.casadeprestamoshyo.com/docs/a4/'+data.tipo_comprobante_id+'-'+data.serie+'-'+data.correlativo+'.pdf';
