                      ...
                      <div class="mt-5">
                        ...
                        <table class="table table-bordered">
                            <thead  class="text-center thead-dark">
                                <tr>
                                    <td rowspan="2">Jefe Comercial</td>
                                    <td rowspan="2">Cod Ejecutivo</td>
                                    <td rowspan="2">Nombre Ejecutivo</td>
                                    <td rowspan="2"># Prospectos Filtdados (Central de Riesgos)</td>
                                    <td colspan="2"># Solicitudes en Proceso</td>
                                    <td colspan="2"># Solicitudes Ingresadas a Créditos</td>
                                    <td colspan="2"># Solicitudes Aprobadas por Créditos</td>
                                    <td colspan="2"># Solicitudes Desembolsadas</td>
                            ...
                            <tbody>
                              <tr v-if="resumen.supervisor">
                                <td colspan="3">{{ resumen.supervisor.nombres }}</td>
                                <td></td>
                                <td>{{ resumen.supervisor.cant_total_pendientes }}</td>
                                <td>{{ resumen.supervisor.total_pendientes }}</td>
                                <td>{{ resumen.supervisor.cant_total_pendientes }}</td>
                                <td>{{ resumen.supervisor.total_pendientes }}</td>
                                <td>{{ resumen.supervisor.cant_total_aprobadas }}</td>
                                <td>{{ resumen.supervisor.total_aprobadas }}</td>
                                <td>{{ resumen.supervisor.cant_total_desembolsadas }}</td>
                                <td>{{ resumen.supervisor.total_desembolsadas }}</td>
                                <td>{{ resumen.supervisor.meta }}</td>
                                <td>{{ resumen.supervisor.avance  }} %</td>
                                <!-- <td v-if="i.total_desembolsadas != 0 && i.meta != 0">{{ i.avance = (parseFloat(i.total_desembolsadas) * 100)/parseFloat(i.meta) }} %</td>
                                <td v-else>{{ i.avance = 0 }} % </td> -->
                              </tr>
                              <tr v-for="i in resumen.analistas">
                                <!-- <td colspan="3">JEFE COMERCIAL</td> -->
                                <td></td>
                                <td></td>
                                <td>{{ i.nombres }}</td>
                                <td></td>
                                <td>{{ i.cant_total_pendientes }}</td>
                                <td>{{ i.total_pendientes }}</td>
                                <td>{{ i.cant_total_pendientes }}</td>
                                <td>{{ i.total_pendientes }}</td>
                                <td>{{ i.cant_total_aprobadas }}</td>
                                <td>{{ i.total_aprobadas }}</td>
                                <td>{{ i.cant_total_desembolsadas }}</td>
                                <td>{{ i.total_desembolsadas }}</td>
                                <td>{{ i.meta }}</td>
                                <td v-if="i.total_desembolsadas != 0 && i.meta != 0">{{ i.avance = (parseFloat(i.total_desembolsadas) * 100)/parseFloat(i.meta) }} %</td>
                                <td v-else>{{ i.avance = 0 }} % </td>
                              </tr>
    ...
    export default {
        ...
        methods: {
          ...
          get(){
            ...
            axios.get('/api/controlventascreditos',{params:{anio:this.anio, mes:this.mes}})
            .then(({data}) => {
              console.log(data);
              let sumaT = 0;
              this.resumen = data;

              data.analistas.map(i=>{
                if (i.total_desembolsadas != 0 && i.meta != 0) {
                  sumaT = sumaT + ((parseFloat(i.total_desembolsadas) * 100)/parseFloat(i.meta));
                } else{
                  sumaT = sumaT + 0;
                }
              })

              this.resumen.supervisor.avance = sumaT/data.analistas.length;
